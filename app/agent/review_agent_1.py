...
from app.model.review import Issue, IssuePatch
...
json_parser=JsonOutputParser(pydantic_object=EmitIssuesArgs)
fixing_output_parser=OutputFixingParser.from_llm(parser=json_parser,llm=llm)
'''
...
async def fetch_node(state: ReviewState, filesvc: FileService) -> ReviewState:
    ...
    return {"headSha": head, "files": files[:MAX_FILES], "contents": contents, "changed": changed, "retry_count": 0}
...
async def review_node(state: ReviewState) -> ReviewState:
    ...
    async def review_file(f: str) -> List[Issue]:
        text = contents.get(f, "")
        if not text:
            return []
        ...
        chain = FILE_TMPL | llm | fixing_output_parser
        ...
        async with sem:
            try:
                out_dict = await with_retries(call)
                tool_issues = [ToolIssue(**i) for i in out_dict.get("issues", [])]
            except Exception as e:
                # After retries, parsing still failed. Skip this file.
                print(f"Skipping file {f} due to parsing error: {e}")
                tool_issues = []

            # Convert ToolIssue objects to the Issue domain model
            issues: List[Issue] = []
            for ti in tool_issues:
                patch = IssuePatch(**ti.patch.dict()) if ti.patch else None
                issues.append(Issue(
                    id=f"AI-{f}-{ti.line or 0}-{ti.title[:20]}",
                    file=f,
                    line=ti.line,
                    title=ti.title,
            ...
            if only_changed and changed:
                filtered = [i for i in issues if (i.line or 0) in changed]
                return filtered
            return issues
...
VERIFIER_TMPL = ChatPromptTemplate.from_messages([
    ("system", "You are a senior software engineer. Your task is to verify the output of a code review agent. The agent has provided a list of issues. Your job is to assess the quality of this review. The review is considered 'good' if it is not empty and all major issues have a patch. It is 'bad' otherwise. Respond with a single word: 'good' or 'bad'."),
    ("human", "Here are the issues found by the review agent:\n\n{issues_str}\n\nIs this a 'good' or 'bad' review?")
])

class VerificationResult(BaseModel):
    quality: Literal["good", "bad"]

async def verify_review_node(state: ReviewState) -> ReviewState:
    issues = state.get("issues", [])
    if not issues:
        return {"review_quality": "bad", "issues": [], "retry_count": state.get("retry_count", 0) + 1}

    issues_str = "\n".join([f"- {i.severity.upper()}: {i.title} (Patch: {'Yes' if i.patch else 'No'})" for i in issues])
    
    verifier_chain = VERIFIER_TMPL | llm_openai.with_structured_output(VerificationResult)
    
    result = await verifier_chain.ainvoke({"issues_str": issues_str})

    print(f"---------Results after verification {result}")    
    
    # If the review is bad, we clear the issues for the next attempt
...
def build_review_graph(filesvc: FileService):
    ...
    graph.add_node("verify_review", verify_review_node)
    ...
    graph.add_edge("review", "verify_review")

    def should_retry(state: ReviewState) -> str:
        if state.get("review_quality") == "bad" and state.get("retry_count", 0) < 2:
            return "review"
        return "dedupe"
